...
var determineWinner = function () {
  ...
  // If neither got 21 but either one goes bust, then announce winner
    ...
    determineBust("player") == true &&
    determineBust("dealer") == false
  ) {
    message = "You're bust! <br>" + dealerWinMsg;
    // Increment dealer wins
    numberOfWins_dealer += 1;
  } else if (
    determineBust("player") == false &&
    determineBust("dealer") == true
  ) {
  ...
  // Otherwise, it is a tie
    ...
    (determineBust("player") == true && determineBust("dealer") == true) ||
    dealerRank == playerRank
  ) {
...
var main = function (input) {
  ...
  if (gameMode == GAME_MODE_PLAYER_HIT) {
    ...
    var playerCardMessage =
      "Player draws " + playerCard.name + " of " + playerCard.suit + ".<br>";
    ...
    if (determineBust("player") == true) {
      hitOrStandMessage =
        "Oops! You bust and should not continue hitting. Enter 'stand' for the dealer's turn.";
    }
    myOutputValue =
      playerCardMessage +
      listOfCards_player +
  ...
  if (gameMode == GAME_MODE_EVALUATE_WIN) {
    ...
    myOutputValue =
      listOfCards_player +
      `Total rank: ${sumOfRanks(playerCardsArray)} <br><br>` +
      listOfCards_dealer +
      `Total rank: ${sumOfRanks(dealerCardsArray)} <br><br>`;
