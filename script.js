...
var MODE_PLAY_GAME = 'MODE_PLAY_GAME';

// Store player's cards and computer's cards in separate arrays
var playerHand = [];
var computerHand = [];
var mode = MODE_QUERY_PLAYER_NAME;
...
var getDefaultOutput = function () {
  return `Player's hand: <br>
  ${convertHandToString(playerHand)} <br>
  Sum: ${getHandSum(playerHand)} 
  <br><br>
...
var main = function (input) {
  ...
  if (mode == MODE_QUERY_PLAYER_NAME) {
    // Prompt user again if input is empty. The trim() method allows removes empty spaces before and after a word. Hence if the user inputs a series of empty spaces, it be trimemed to a single space i.e. ''
    if (input.trim() == '') {
      return 'Please input your name to begin';
    }
    mode = MODE_PLAY_GAME;
    playerName = input;
    return 'Welcome, ' + playerName + '! Click submit to deal your cards.';
  }
