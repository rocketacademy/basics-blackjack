  ...
  function generateDeck(){
    ...
    for(i=0; i<52;i++){
      ...
      newCard.value = (newCard.property == 'Jack' || newCard.property == 'Queen' || newCard.property == 'King') ? 10 : newCard.property == 'Ace' ? 1 : Number(newCard.property);
  ...
  var blackjack = function(hand){
    ...
    if (score(hand)==11 && (hand[0].property == 'Ace' || hand[1].property == 'Ace')){
      return true;
    } else {
      return false;
    }
  ...
  var hit = function(){
    ...
    console.log(newCard);
    var playerDraw = '<br>' + '<br>' + 'You drew the ' + newCard.name + ' for a total of ' + score(playerHand)+'.';
  ...
  var stand = function(){
    if (score(playerHand) < 17) {
      ...
      hitme.removeEventListener('click',hit);
      var endTurn = '<br>' + '<br>' +'The house opens the ' + houseHand[1].name + ' with a total score of ' + score(houseHand) +'.';
      output.innerHTML += endTurn;

      while(score(houseHand) < 17) {
        newCard = deal();
        houseHand.push(newCard);
        var houseDraw = '<br>'+'<br>' +'The house draws a ' + houseHand[0].name + ' for a sum of ' + score(houseHand)+'.';
  ...
  var startGame = function(){
    ...
    var firstround = 'You were dealt the ' + playerHand[0].name + ' as well as the ' + playerHand[1].name + ', for a total score of ' + score(playerHand) +'.';
    ...
    if (blackjack(playerHand)){
      ...
      if(blackjack(houseHand)){
        var draw = '<br>' +'<br>' +'Too bad, the house deals a BlackJack too! It\'s a draw!'
