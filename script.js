...
let checkForBlackJack = function (handArray) {
    ...
    if ((playerCardOne.name == `Ace` && playerCardTwo.rank >= 10) || (playerCardOne >= 10 && playerCardTwo.rank == `Ace`)) {
        hasBlackJack = true;
    }
    return hasBlackJack;
};
...
let showHandTotalSum = function (playerHandSum, dealerHandSum) {
    let handTotalSumMessage = `<br>Player total hand value: ` + playerHandSum + `<br>Dealer total hand value: ` + dealerHandSum;
...
let main = function (input) {
    ...
    if (currentGameState == GAME_STATE_HIT_OR_STAND) {
        if (input == `hit`) {
            playerHand.push(gameDeck.pop());
            outputMessage = showPlayerAndDealerHands(playerHand, dealerHand) + `<br>You drew another card. Choose <b>hit</b> or <b>stand</b>.`;
            ...
            if (playerHandSum == dealerHandSum) {
                outputMessage = showPlayerAndDealerHands(playerHand, dealerHand) + `<br>` + `It's a tie!<br>` + showHandTotalSum(playerHandSum, dealerHandSum);
            } else if (playerHandSum > 21 && dealerHandSum > 21) {
                outputMessage = showPlayerAndDealerHands(playerHand, dealerHand) + `<br>` + `<b>BOOM!</b> Both dealer and player busted!` + showHandTotalSum(playerHandSum, dealerHandSum);
            } else if (playerHandSum > 21) {
                outputMessage = showPlayerAndDealerHands(playerHand, dealerHand) + `<br>` + `<b>Dealer wins!</b><br>` + showHandTotalSum(playerHandSum, dealerHandSum);
            } else if (dealerHandSum > 21) {
                outputMessage = showPlayerAndDealerHands(playerHand, dealerHand) + `<br>` + `<b>Player wins!</b><br>` + showHandTotalSum(playerHandSum, dealerHandSum);
