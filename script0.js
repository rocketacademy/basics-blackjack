...
const playBlackJack = function () {
  if (mode === "start mode") {
    ...
    for (let playerIndex = 0; playerIndex <= totalPlayers - 1; playerIndex++) {
      ...
      sumOfHandPlayer =
        cardsOfPlayer[playerIndex].value + cardsOfPlayer[playerIndex + 1].value;
      // push sum into array
      ...
      dealerDeck = playDeck;
      if (
        (cardsOfPlayer[0].value === 10 && cardsOfPlayer[1].name === "ace") ||
    ...
    //create object
  //   const recordParticipants = Object.fromEntries(
  //     players.map((key) => [key, handOfPlayer])
  //   );
  ...
  if (mode == "dealer mode") {
    ...
    console.log(
      ...
      cardsOfDealer,
      `and the dealer sum is `,
      sumOfDealer
    );
    console.log(`Remaining Deck now has`, dealerDeck.length, `cards.`);

    if (
      (cardsOfDealer[0].value === 10 && cardsOfDealer[1].name === "ace") ||
...
const isBlackJack = function(card1, card2){
