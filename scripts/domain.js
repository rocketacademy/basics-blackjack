...
class Round {
  ...
  _resetTurn = () => {
    this._nextPlayer = this._nextActorGenerator(this.getPlayers());
  };
  /**
   *
   * @returns {RoundPhase}
  ...
  changePhase = (phase) => {
    ...
    } else if (this._phase === RoundPhase.BET) {
  ...
  _autoDeal = () => {
    ...
    let actor = dealPlayersOrder();
    while (actor) {
      console.log("deal to " + actor.getName() + " hands");
